pub mod test;
pub mod operations {
    pub mod armstrong;
    pub mod automorphic;
    pub mod matrix;
    pub mod neon;
    pub mod palindrome;
}
use crate::operations::armstrong::armstrong;
use crate::operations::automorphic::automorphic;
use crate::operations::matrix::Matrix;
use crate::operations::neon::neon;
use crate::operations::palindrome::palindrome;
use log::*;
use text_io::read;

fn main() {
    env_logger::init();
    loop {
        info!(
            "------Operations------ \n
    1:- Check the given number is Armstrong number or not\n
    2:- Check the given number is Automorphic number or not\n
    3:- Check the given number is Neon number or not\n
    4:- Check the given number is Palindrome number or not\n
    5:- Perform matrix multiplication\n
    (Press -1 to exit)\n"
        );
        let choice: i32 = read!();
        if choice == 1 {
            info!("Enter a number",);
            let number: i32 = read!();
            debug!("{}", armstrong(number));
        }
        if choice == 2 {
            info!("Enter a number");
            let number: i32 = read!();
            debug!("{}", automorphic(number));
        }
        if choice == 3 {
            info!("Enter a number");
            let number: i32 = read!();
            debug!("{}", neon(number));
        }
        if choice == 4 {
            info!("Enter a number");
            let number: i32 = read!();
            debug!("{}", palindrome(number));
        }
        if choice == 5 {
            let first = Matrix {
                element: [[1, 2, 3], [4, 5, 6], [7, 8, 9]],
            };
            let second = Matrix {
                element: [[1, 0, 0], [0, 1, 0], [0, 0, 1]],
            };
            let result = Matrix::matrix_multiplication(first, second);
            debug!("{:?}", result);
        }
        if choice == -1 {
            break;
        }
    }
}
